...
class ArgsRecoder():
    ...
    def _mark_shorter(self,key,value):
        if type(value)==list:
            ...
            raise ValueError(f"Unsupported mark type:{key} {value} {type(value)}")
        return buf
...
parser_recoder('data_random_seed',
    df_group.add_argument("--data_random_seed",type=int,help='$1'))
parser_recoder('data_dividing_rates',
    df_group.add_argument("--data_dividing_rates",type=float,nargs='+',help='$1'))
parser_recoder('data_dividing_seed',
    df_group.add_argument("--data_dividing_seed",type=int,help='$1'))
parser_recoder('norm',
...
parser_recoder('cut_ranges',
    df_group.add_argument("--cut_ranges",type=int,nargs='+',action='append',default=((69,84),(0,239),(0,239))))
parser_recoder('patch_sizes',
    df_group.add_argument("--patch_sizes",type=int,nargs='+',default=(3,128,128)))
parser_recoder('batch_size',
    df_group.add_argument("--batch_size",type=int,default=1))
parser_recoder('patch_nums',
    df_group.add_argument("--patch_nums",type=int, nargs='+',default=(6,3,3)))
parser_recoder('domain',
    df_group.add_argument("--domain", type=float, nargs='+',default=[0,1.0]))
#----------------------------------------------------------------------------------------#
