...
class UpSampalingConv(tf.keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 strides:Union[Tuple,List,int]=1,
                 padding:str='same',
                 data_format:str='channels_last',
                 dilation_rate:Union[Tuple,List,int]=1,
                 groups:int=1,
                 activation=None,
                 use_bias:bool=True,
        ...
        #     raise ValueError("rank should be a integer of 1 or 2 or 3, not {}.".format(_rank)) #NOTE there is no need because this Class is not open to user
        ...
        if (_interpolation not in ['nearest','bilinear'])or(_rank!=2 and _interpolation != "nearest"):
            ...
            "interpolation, `UpSampling2D` only  support 'nearest' or 'bilinear' interpolation.".format(interpolation))           
        ...
        if any([x!=1 for x in _strides]):
            raise ValueError("UpSampalingConv's strides should be 1, not {}.".format(strides))
        _convolution_kwargs["strides"] = _strides
        _padding = padding.lower()
        if _padding != "same":
            raise ValueError("UpSampalingConv's conv padding should be 'same', not '{}'.".format(padding))
        ...
        if any([x!=1 for x in dilation_rate]):
            logging.warning("UpSampalingConv ususally runs with dilation_rate==1, please be careful whether "
                "the operation is your desired one when dilation_rate={}.".format(dilation_rate))
        ...
        if _rank == 1:
            ...
            raise ValueError("rank should be one of 1, 2 or 3, not {}.".format(_rank))
