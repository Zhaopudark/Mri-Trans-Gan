...
class DiscriminatorHelper():
    ...
    def output_sigmoid_flag(self):
        if self.gan_loss_name.lower() in ['wgan',"wgan-gp"]:
            logging.getLogger(__name__).info("Discriminator's last layer will not be limited (clipped) by sigmoid")
            return False
        elif self.gan_loss_name.lower() in ['vanilla','lsgan','rsgan','hinge']:
            logging.getLogger(__name__).info("Discriminator's last layer will be limited (clipped) by sigmoid")
            return True
        else: 
